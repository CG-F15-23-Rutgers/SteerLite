...
Util::Vector SocialForcesAgent::calcProximityForce(float dt)
{
	...
	if ((_position.x <= 10 && _position.x >= -10) && (_position.z <= 10 && _position.z >= -10))
		{

			Util::Vector away_tmp = normalize(position() - Util::Point(0, 0, 0));
	...
	if ((_position.x >= -16 && _position.x <= -8) && (_position.z >= -16 && _position.z <= -8))
		{

			Util::Vector away_tmp = normalize(position() - Util::Point(-12, 0, -12));
...
{
	return (((_goalDirection*PREFERED_SPEED) - velocity()) / _dt)/4;
}
		...
		if ((*neighbor)->isAgent())
		{
			...
			if ((id() != tmp_agent->id()) && (tmp_agent->computePenetration(this->position(), this->radius()) > 0.000001))
			{
				agent_repulsion_force = agent_repulsion_force +
					tmp_agent->computePenetration(this->position(), this->radius()) * _SocialForcesParams.sf_agent_body_force * normalize(position() - tmp_agent->position());
			}
		}
		else
		{
...
Util::Vector SocialForcesAgent::calcWallRepulsionForce(float dt)
{
	...
	if ((_position.x <= 10 && _position.x >= -10) && (_position.z <= 10 && _position.z >= -10)){
		if (Util::computeCircleCirclePenetration2D(_position,this->_radius,Util::Point(0, 0, 0), 5) > 0.000001)
			{
				wall_repulsion_force = wall_repulsion_force +
					Util::computeCircleCirclePenetration2D(_position,this->_radius,Util::Point(0, 0, 0), 5)  * _SocialForcesParams.sf_body_force * normalize(position() - Util::Point(0, 0, 0));
			}
	}

	if ((_position.x >= -16 && _position.x <= -8) && (_position.z >= -16 && _position.z <= -8)){
		if (Util::computeCircleCirclePenetration2D(_position,this->_radius,Util::Point(-12, 0, -12), 4) > 0.000001)
			{
				wall_repulsion_force = wall_repulsion_force +
					Util::computeCircleCirclePenetration2D(_position,this->_radius,Util::Point(-12, 0, -12), 4)  * _SocialForcesParams.sf_body_force * normalize(position() - Util::Point(-12, 0, -12));
			}
	}
	...
	for (std::set<SteerLib::SpatialDatabaseItemPtr>::iterator neighbor = _neighbors.begin(); neighbor != _neighbors.end(); neighbor++)
	{
		...
		if (!(*neighbor)->isAgent())
		{
			...
			if (tmp_ob->computePenetration(this->position(), this->radius()) > 0.000001)
			{
				...
				wall_repulsion_force = wall_repulsion_force + wall_normal * (min_stuff.first + radius())*_SocialForcesParams.sf_body_force*dt;

				wall_repulsion_force = wall_repulsion_force + tmp_ob->computePenetration(this->position(), this->radius()) *
					_SocialForcesParams.sf_sliding_friction_force *
					dot(normalize(velocity()), rightSideInXZPlane(wall_normal)) *
