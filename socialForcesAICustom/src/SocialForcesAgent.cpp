...
void SocialForcesAgent::reset(const SteerLib::AgentInitialConditions & initialConditions, SteerLib::EngineInterface * engineInfo)
{
	...
	if (agentid>41)
	{
		...
		_startPosition = initialConditions.position;
		...
		std::queue<SteerLib::AgentGoalInfo> goalQueue = _goalQueue;
		for (int i = 0; i < _goalQueue.size(); i++)
		{
			controlPoints.push_back(Util::CurvePoint(goalQueue.front().targetLocation, goalQueue.front().targetTangent, goalQueue.front().targetTime));
			goalQueue.pop();
		}
...
void SocialForcesAgent::updateAI(float timeStamp, float dt, unsigned int frameNumber)
{
	...
	if (agentid>41)
	{
		std::cout << "AGENT IS " << agentid << std::endl;
	...
	if (agentid>41 )
	{
		...
		if (gEngine->isAgentSelected(this)) {
			...
			if (gSpatialDatabase->trace(ray, t, objectFound, this, false)) {
				Util::DrawLib::drawAgentDisc(_position, _forward, _radius, Util::gOrange);
			}
			else {
				Util::DrawLib::drawAgentDisc(_position, _forward, _radius, Util::gDarkOrange);
			}
		}
		else {
			Util::DrawLib::drawAgentDisc(_position, _forward, _radius, agentColor);
		}
		...
		for (int i = 0; i < _goalQueue.size(); i++)
		{
			if (goalQueue.front().goalType == SteerLib::GOAL_TYPE_SEEK_STATIC_TARGET)
			{
				Util::DrawLib::drawFlag(_goalQueue.front().targetLocation, agentColor, 2);
			}
			goalQueue.pop();
		}
